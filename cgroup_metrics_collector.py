...
class CgroupMetricsCollector:
    ...
    def _detect_cgroup_version(self) -> int:
        ...
        elif Path('/sys/fs/cgroup/cpu').exists() or Path('/sys/fs/cgroup/memory').exists():
            return 1
        else:
    ...
    def _get_cpu_usage_v1(self) -> Optional[float]:
        ...
        if not usage_str:
            ...
            usage_str = self._read_cgroup_file('/sys/fs/cgroup/cpu,cpuacct/cpuacct.usage')
    ...
    def _get_memory_usage_v1(self) -> Optional[float]:
        ...
        # Read current memory usage in bytes
        memory_str = self._read_cgroup_file('/sys/fs/cgroup/memory/memory.usage_in_bytes')
        if not memory_str:
            return None
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    LOGGER.debug(f"Successfully pushed metrics for {self.container_name}: CPU={metrics['cpu_usage']:.4f} cores, RAM={metrics['memory_usage']:.2f} MB")
                    return True
                else:
...
async def start_cgroup_metrics_collector(container_id: int, container_name: str, core_backend_url: Optional[str] = None) -> CgroupMetricsCollector:
    """
    ...
    LOGGER.info(f"Starting cgroup metrics collector for {container_name} pushing to {core_backend_url}")
    
    collector = CgroupMetricsCollector(container_id, container_name, core_backend_url)
